"use client";

import React, { useState } from "react";
import { createClient, SupabaseClient } from "@supabase/supabase-js";

interface CloudCreateProps {
  nickname: string;
  onRoomCreated: (roomId: string, roomName: string) => void;
  onBack: () => void;
}

export default function CloudCreate({
  nickname,
  onRoomCreated,
  onBack,
}: CloudCreateProps) {
  const [roomName, setRoomName] = useState("");
  const [creating, setCreating] = useState(false);
  const [error, setError] = useState("");

  // Create a supabase client
  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const supabaseKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
  const supabase: SupabaseClient = createClient(supabaseUrl!, supabaseKey!);

  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();

    if (!roomName.trim()) {
      setError("Please enter a cloud name");
      return;
    }

    if (!nickname.trim()) {
      setError("Please enter your nickname first");
      return;
    }

    try {
      setCreating(true);
      setError("");

      // Create a new room
      const { data: room, error: roomError } = await supabase
        .from("cloud_rooms")
        .insert([
          {
            name: roomName.trim(),
          },
        ])
        .select()
        .single();

      if (roomError) throw roomError;

      // Add the creator as a participant
      const { error: participantError } = await supabase
        .from("cloud_participants")
        .insert([
          {
            room_id: room.id,
            nickname: nickname.trim(),
          },
        ]);

      if (participantError) throw participantError;

      // Navigate to the newly created room
      onRoomCreated(room.id, room.name);
    } catch (error: any) {
      console.error("Error creating cloud:", error);
      setError(error.message || "Failed to create cloud. Please try again.");
    } finally {
      setCreating(false);
    }
  };

  return (
    <div className="flex flex-col h-full bg-[#f8f9fa] text-[#495057] font-serif">
      <div className="flex items-center px-4 py-3 border-b border-gray-200">
        <button
          onClick={onBack}
          className="text-[#495057] p-2 hover:bg-gray-100 rounded-full"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="w-5 h-5"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              strokeWidth={2}
              d="M15 19l-7-7 7-7"
            />
          </svg>
        </button>
        <h1 className="ml-2 text-xl font-bold text-[#4dd783]">Create Cloud</h1>
      </div>

      <div className="flex-1 p-6">
        <div className="mx-auto max-w-md">
          <div className="mb-8 text-center">
            <div className="inline-block p-4 bg-[#f1f8e9] rounded-full mb-4">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-10 w-10 text-[#4dd783]"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
                <path
                  d="M5.03426 11.1174C3.29168 11.5494 2 13.1238 2 15C2 17.2091 3.79086 19 6 19H17C19.7614 19 22 16.7614 22 14C22 11.2386 19.7614 9 17 9C16.5971 9 16.2053 9.04766 15.83 9.13765L14.5 9.5"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
                <path
                  d="M15.83 9.13765C15.2238 6.75905 13.0673 5 10.5 5C7.46243 5 5 7.46243 5 10.5C5 10.7087 5.01163 10.9147 5.03426 11.1174C5.03426 11.1174 5.1875 12 5.5 12.5"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
              </svg>
            </div>
            <h2 className="text-xl font-bold text-[#212529] mb-2">
              Create a New Cloud
            </h2>
            <p className="text-[#6c757d]">
              Your own private space to connect with friends
            </p>
          </div>

          {error && (
            <div className="p-3 mb-4 text-red-700 bg-red-50 rounded-lg border border-red-100">
              {error}
            </div>
          )}

          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <label
                htmlFor="roomName"
                className="block text-[#495057] font-medium"
              >
                Cloud Name
              </label>
              <input
                id="roomName"
                type="text"
                value={roomName}
                onChange={(e) => setRoomName(e.target.value)}
                placeholder="e.g., Weekend Hangout"
                className="w-full bg-white text-[#495057] px-4 py-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#4dd783] border border-gray-200"
                maxLength={40}
                disabled={creating}
              />
              <p className="text-xs text-[#6c757d]">
                Give your cloud a name that your friends will recognize
              </p>
            </div>

            <button
              type="submit"
              disabled={creating || !roomName.trim() || !nickname.trim()}
              className="w-full bg-[#4dd783] text-white py-3 rounded-lg font-bold disabled:opacity-50 hover:bg-[#3bb871] transition-colors"
            >
              {creating ? (
                <span className="flex justify-center items-center">
                  <svg
                    className="mr-2 -ml-1 w-4 h-4 text-white animate-spin"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                  >
                    <circle
                      className="opacity-25"
                      cx="12"
                      cy="12"
                      r="10"
                      stroke="currentColor"
                      strokeWidth="4"
                    ></circle>
                    <path
                      className="opacity-75"
                      fill="currentColor"
                      d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"
                    ></path>
                  </svg>
                  Creating...
                </span>
              ) : (
                "Create Cloud"
              )}
            </button>
          </form>
        </div>
      </div>
    </div>
  );
}
